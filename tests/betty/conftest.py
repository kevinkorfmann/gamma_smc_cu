...
def pop_map(samples_txt):
    """Map sample_id -> population code.

    samples.txt is the 1000G header-prefixed space-separated file with columns:
    ...
    with open(samples_txt) as f:
        ...
        for line in f:
            parts = line.split()
            if len(parts) > max(pop_col, sample_col):
                m[parts[sample_col]] = parts[pop_col]
    return m
