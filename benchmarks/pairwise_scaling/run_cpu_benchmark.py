...
print(f"=== CPU benchmark: chr{CHR} {POP} ===", flush=True)
print(f"Host: {os.popen('hostname').read().strip()}", flush=True)
print(f"CPU: {os.popen('lscpu | grep \"Model name\"').read().strip()}", flush=True)
G_pop, pos = load_data()
n = G_pop.shape[0]
...
with tempfile.TemporaryDirectory(prefix="gsmc_") as td:
    for nh in hap_counts:
        ...
        if r.returncode != 0:
            print(f"  {nh:>3} haps FAILED", flush=True)
            continue
        results.append(("gamma_smc", np_, elapsed, "measured"))
        print(f"  {nh:>3} haps ({np_:>6} pairs): {elapsed:.3f}s ({elapsed/np_:.4f}s/pair)", flush=True)
...
try:
    ...
    with tempfile.TemporaryDirectory(prefix="asmc_") as td:
        ...
        others = [i for i in range(n) if i not in (pair_a, pair_b)]
...
csv_path = os.path.join(OUT_DIR, "results_cpu.csv")
