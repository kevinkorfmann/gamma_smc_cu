...
The backward pass is not a GPU-specific algorithmic innovation---Schweiger's tool could in principle add one and achieve similar accuracy gains.
However, doing so would approximately double the CPU runtime (the backward scan is symmetric to the forward), making it unattractive for the large pair counts where the improvement matters most.
On GPU, the backward pass adds negligible wall-clock time because the pairs are already distributed across thousands of threads and the cached flow field data remains in L2 cache from the forward pass.
