...
These speedups bring all-pairs TMRCA estimation for a 1\,Mb region across 1{,}000 haplotypes---roughly 500{,}000 pairs---from 89 seconds on CPU down to 0.74 seconds on GPU.
...
On a single NVIDIA A100 GPU, \texttt{tmrca.cu} estimates coalescence times for all half-million pairs among 1{,}000 haplotypes in under 0.8 seconds---a 121-fold speedup.
...
On a single NVIDIA A100, \texttt{tmrca.cu} achieves 121--3{,}145$\times$ speedups over the original Gamma-SMC binary while simultaneously improving estimation accuracy from $r=0.791$ to $r=0.820$ (Pearson correlation against ground truth on a log scale).
...
On a single NVIDIA A100, it achieves 121--3{,}145$\times$ speedups over the original CPU implementation while producing more accurate estimates through the use of a full forward--backward algorithm.
...
We have implemented multi-GPU parallelism that distributes pairs across devices via concurrent threads, each backed by its own \texttt{FlowContext} with device-local memory allocations and GIL-released CUDA operations.  Since pairs are independent, this yields near-linear scaling: three A100 GPUs provide $2.7\times$ throughput over a single device for large workloads (Table~\ref{tab:multigpu}), with no change to the underlying CUDA kernels.
